...
#----------------------------------------------------
'''
    (Random Sample) Given the total number of photos (n), randomly choose a pair of
    the photo IDs (A and B) to return. Here we assume the IDs of all
    the photos are: 0,1,2,..., n-1. For example, if we have 5 photos (n=5), the IDs of the
    photos will be: 0,1,2,3,4. We then randomly sample two IDs (such as A=2, B=0), where
    the two IDs should be different from each other. 
    ---- Inputs: --------
...
#----------------------------------------------------
'''
    (Homepage) Create the homepage of facemash. Each time when a user visit the URL '/',
    randomly sample two photo IDs (A and B), and use these IDs and the template to render
    the webpage. Here the 'template' variable (a global variable) was already created in
    line #5 of this file. You could use this template to render the webpage
    using the IDs (A and B). 
    ---- Outputs: --------
...
def facemash():
    ...
    photos = sample_pair(8)
    webpage = template.render(A=photos[0], B=photos[1])
    #########################################
    return webpage
    #-----------------
    '''  
...
#----------------------------------------------------
'''
    (Vote Page) Create a page for user to vote. For example, if a user visits (/vote/4/3),
    it means that the user was given two photos with ID 3 and 4. This user voted Photo
    4 wins, Photo 3 loses. In this function, please (1) update the game result
    into the list (data), then (2) redirect the page to the homepage of facemash. 
	 (1) In line #6 of this file, we have created an empty list (called 'data') to store
    all the vote data. Please store the game result by add a tuple of (3,4) into the list (data). 
	 For example, at the beginning, the list is data = [], 
	 Suppose a user voted Photo 5 wins, Photo 3 loses, we need to add a tuple (5,3) to the
    data list, so now data = [(5,3)] 
	 Suppose another user voted Photo 0 wins and Photo 2 loses, we need to add
    a tuple (0,2) to the data list, so now data = [(5,3),(0,2)] 
	 (2) In the returned webpage, please redirect the user to the homepage of facemash . 
    ---- Inputs: --------
        * win_id: .
        * lose_id: .
    ---- Outputs: --------
        * webpage: a webpage returned to the user's browser to render, a string of plain
    text or html code.
    ---- Hints: --------
