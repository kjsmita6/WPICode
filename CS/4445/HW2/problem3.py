...
'''
    ...
    In this problem, you will implement the Elo rating algorithm to rank players based
    upon match results (win/lose). Assuming all the players' performance are random following a
    Logistic distribution with a scale of 400
...
'''
    ...
    ---- Outputs: --------
        * EA: the expected probability of player A wins when competing with player B in a game,
    a float scalar value between 0 and 1.
    ---- Hints: --------
...
#----------------------------------------------------
'''
    Given the expected probability of player A (with rating RA) to win in a game against a
    player B (with rating RB). 
    ---- Inputs: --------
        * EA: the expected probability of player A wins when competing with player B in a game,
        a float scalar value between 0 and 1.
    ---- Outputs: --------
        * EB: the expected probability of player B wins when competing with player A in a game,
        a float scalar value between 0 and 1.
    ---- Hints: --------
        * This problem can be solved using 1 line(s) of code.
'''
...
#----------------------------------------------------
'''
    Given the result of one game between player A and player B, the current ratings of both
    players, compute the new rating of player A. 
    ---- Inputs: --------
        * RA: the rating of player A (the average performance of the player), a float scalar value.
        * SA: the result of one game where player A is competing with player B, a scalar value.
        If player A wins in this game, SA = 1; if player A loses, SA =0.
        * EA: the expected probability of player A wins when competing with player B in a game,
        a float scalar value between 0 and 1.
        * K: k-factor, a constant number which controls how maximum amount of change that can be
        applied on the rating of a player based upon the result of one game.
    ---- Outputs: --------
        * RA: the rating of player A (the average performance of the player), a float scalar value.
    ---- Hints: --------
        * This problem can be solved using 1 line(s) of code.
