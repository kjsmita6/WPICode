...
'''
 ...
 we should sum the data for each player into one row: 
	...
	----------------------------------------- 
	  ...
	  player 3 | 1          | 60 
	----------------------------------------- 
 (Group by playerID) Given a data frame of batting statistics (X1), group the data records with
    respect to playerID, so that the game statistics are added together for each player.
    For example, player 'houstty01' has two rows, where the number of hits (column H) has
    values: 58, 4 We want to combine these two rows into one row, such that all the game
    statistics are the sum of the raw values (for example, number hits now should be 58+4 = 62) .
    ---- Inputs: --------
        * X1: a dataframe containing the batting data only in the searched year (2001).
    ---- Outputs: --------
...
#----------------------------------------------------
'''
    If you have passed the previous test case, the result data frame should have been saved
    into a file, called 'moneyball_Z1.csv'. Now let's merge the salary information into
    the dataset. 
 (Join the batting data with salary data) Given a data frame X3
    (containing both batting statistics and player information, loaded from 'moneyball_X3.csv'),
    and a dataframe (Z1) of salary information (loaded from 'moneyball_Z1.csv'), combine
    the two data frames into one, according to the 'playerID' column. 
    ---- Inputs: --------
        * X3: a dataframe containing both batting data and player information in the year (2001).
        * Z1: a dataframe containing the salary data only in the searched year (2002).
    ---- Outputs: --------
...
def filter_max_salary(X5, max_salary):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    X6 = X5.query('salary <= ' + str(max_salary))
    #########################################
    return X6
    #-----------------
    '''  
