...
(define (apply  [f : ExprC] [args : (listof ExprC) ] [env : Env]) : Value
  ...
  (let ( (f-value (interp f env))
         (args-values  (map (lambda (a) (interp a env)) args)))
    (cond
      [(not (closV? f-value))
       ;; if f-value not a closure we have a bug
       (error 'apply "type error: attempt to apply a non-closure")]
      
      [else
       ;; extract the parts of the closure
       (let ((params (closV-params f-value))
             (f-bdy  (closV-body f-value))
             (f-env  (closV-env f-value)))
         ;; check that the closure is appropriate for this application
         (cond [(different-length? params args)
                (length-mismatch-error 'interp params args)]
               [(multiples? params)
                (multiple-names-error 'interp (to-string params))]
               
               [else  ;; ok, all is good, let's go
                (let ((new-env (add-bindings params args-values f-env)))
                  (interp f-bdy new-env))]))]
         )))
