...
(define (type-of [e : ExprS] [tenv : typeEnv]) : Type
  (type-case ExprS e
             [idS (i)
                  (lookup-type i tenv)]
             [appS (f arg )
                   (let ((ft (type-of f tenv)) (at (type-of arg tenv)))
                     (if (and (funT? ft) (equal? (funT-input ft) at))
                         (funT-return ft)
                         (type-error)))]
             [lamS (param paramtype rettype body)
                   (if (equal? (type-of body (extend-tenv (tbind param paramtype) tenv)) rettype)
                       (funT paramtype rettype)
                       (type-error))]
             [withS (var val body)
                    (let ((extended-env (extend-tenv (tbind var (type-of val tenv)) tenv)))
                      (type-of body extended-env))]
             [recS (fn val body)
                   (let ((extended-env (extend-tenv (tbind fn (type-of val tenv)) tenv)))
                     (type-of body extended-env))]
             [numS (n)
                   (numT)]
             [boolS (b)
