...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        saveBtn.setOnClickListener {
            val intent = Intent(this, TinoActivity::class.java)
            startActivityForResult(intent, 0)
            Log.d(TAG, "SAVE button pushed, startActivityForResult")
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 0) {
            findViewById<Button>(R.id.btn_reset).performClick()
            val thanks: Boolean = data?.getBooleanExtra(TinoActivity.THANKS_TINO, false) ?: false
            Log.d(TAG, "onActivityResult: thanks=" + (if (thanks) "true" else "false"))
            if (thanks) {
                Toast.makeText(applicationContext, "Tino wishes you luck on your next game!", Toast.LENGTH_LONG).show()
            } else {
                Toast.makeText(applicationContext, "Tino says you are the meanest human he has ever met!", Toast.LENGTH_LONG).show()
            }
        }
    }
}